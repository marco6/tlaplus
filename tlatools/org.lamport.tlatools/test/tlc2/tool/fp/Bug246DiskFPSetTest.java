...
public class Bug246DiskFPSetTest {
	...
	/**
	 * Tests if the DiskFPSet gets correctly flushed to disk (if the fp spaces is
	 * unevenly distributed) or causes an {@link OutOfMemoryError}
	 * 
	 * @throws IOException
	 */
	@Test
	...
	public void testLinearFillup() throws IOException {
		...
		// TODO maxMemoryInBytes actually max amount of fingerprints which technically
		// fit into memory?
		...
		try {
			...
			// Linearly fill DiskFPSet in-memory storage to simulate an unevenly distributed
			// fp space
			for (int i = 0; i < fpSet.getTblCapacity() - 1; i++) {
				...
				// too. This is the case, if values start with Long.MAX_VALUE and decrease by
				// one.
	...
	public void testFlushDiskFPSet() throws IOException {
		...
		// final long vmMaxMemory = Runtime.getRuntime().maxMemory();
		// long maxMemoryInBytes = TLCRuntime.getInstance().getFPMemSize(0.5d);
		// assertTrue("Not enough memory dedicated to JVM, increase -Vmx value",
		// vmMaxMemory > maxMemoryInBytes);
		//
		// final DummyDiskFPSet fpSet = new DummyDiskFPSet(maxMemoryInBytes);
		// fpSet.init(0, System.getProperty("java.io.tmpdir"),
