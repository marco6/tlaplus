...
public class AstNode {
	...
	public static enum Kind {
		 ...
		 */
		private static final Map<String, Kind> nameMap = Arrays
				.stream(Kind.values())
		 ...
		 */
		private static final EnumSet<Kind> pcal = EnumSet.copyOf(
				Arrays
						.stream(Kind.values())
	...
	public void testEquality(AstNode other) {
		...
		for (int i = 0; i < this.children.size(); i++) {
			...
			if (this.fieldNames.containsKey(thisChild)) {
				Assert.assertEquals(this.kind.toString(), this.fieldNames.get(thisChild),
						other.fieldNames.get(otherChild));
			}
