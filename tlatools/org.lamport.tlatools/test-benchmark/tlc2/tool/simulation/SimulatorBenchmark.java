...
public class SimulatorBenchmark {
	...
	public void reseed() {
		...
		// This should help to make these benchmarks more deterministic for a fixed set
		// of benchmark parameters. The initial
		// seed of this random number generator should effectively determine what traces
		// are explored in what order by the simulation
		// workers, so a particular seed should correspond to a fixed exploration order
	...
	public void simulatorBenchmark(int nWorkers) {
		try {
			...
			Simulator simulator = new Simulator("BenchmarkSpec", "MCInv", null, false, maxTraceDepth, Long.MAX_VALUE,
					rng, 0,
