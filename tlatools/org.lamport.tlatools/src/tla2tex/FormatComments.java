...
public final class FormatComments {
   ...
   ***********************************************************************/
  {
    return (com[line][itemNo].column > com[line][itemNo - 1].column +
        com[line][itemNo - 1].getWidth() + 1)
        || ((com[line][itemNo].column > com[line][itemNo - 1].column +
   ...
   ***********************************************************************/
  {
            ...
            && !(Misc.IsLowerCase(tstr)
                ...
                || (Misc.IsCapitalized(tstr)
                    && ((item == 0)
                        ...
                        || (ptok.column + ptok.getWidth() <= tok.column - 2))))) {
          tok.isTLA = true;
        }
    ...
    while (line < com.length) {
      ...
      while (item < com[line].length) {
        ...
        if ((tok.type == CToken.BUILTIN)
            && (BuiltInSymbols.GetBuiltInSymbol(tok.string, true).symbolType == Symbol.LEFT_PAREN)
            && ((parenLevel > 0)
                || tok.isTLA)) {
          ...
          if ((tok.type == CToken.BUILTIN)
              && ((BuiltInSymbols.GetBuiltInSymbol(tok.string, true).symbolType == Symbol.RIGHT_PAREN)
                  || tok.string.equals("]_"))
              && (parenLevel > 0)) {
            parenLevel = parenLevel - 1;
          }
            ...
            case CToken.NUMBER:
              if (Parameters.TLACommentOption) {
                ...
                if (ptok.type == CToken.BUILTIN) {
                  int stype = BuiltInSymbols.GetBuiltInSymbol(ptok.string, true).symbolType;
              ...
              if (ntok.type == CToken.BUILTIN) {
                int stype = BuiltInSymbols.GetBuiltInSymbol(ntok.string, true).symbolType;
                if (((stype == Symbol.INFIX)
              ...
              break;
            case CToken.BUILTIN:
              int stype = BuiltInSymbols.GetBuiltInSymbol(tok.string, true).symbolType;
    ...
    while (line < com.length) {
      ...
      oneLine[line] = thisAligned
          ...
          || (prevAligned
              && (line + 1 < com.length)
              && (com[line + 1].length > 0)
              && (com[line][0].column > com[line + 1][0].column));
      if ((thisAligned)
          && ((line + 2 == com.length)
              || (com[line + 2].length == 0))) {
        oneLine[line + 1] = true;
    ...
    if (IsParType(commentType)) { // BEGIN then OF if (IsParType(commentType))
      ...
      while (line < com.length) { // BEGIN while (line < com.length)
        if (com[line].length == 0) {
          ...
          String temp = "\\begin{" + Parameters.LaTeXCommentPar
          ...
          if (com[line][item].type == CToken.VERB) {
            ...
            if (!isPreviousAdjacent(com, line, item)
              ...
              if ((item > 0)
                ...
                curOutput = curOutput + Parameters.LaTeXSpaceCommand
                    + "{"
                    + Misc.floatToString(
  ...
  private static void CloseParagraph(OutputFileWriter writer,
      String curOutput,
      boolean openMath,
      boolean openVerb)
  /***********************************************************************
