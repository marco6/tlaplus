    ...
    try {
      ...
      try {
        if (Files.exists(iFile)) {
          Files.move(iFile, iFile.resolveSibling(Parameters.LaTeXOutputFile + ".old"),
              StandardCopyOption.REPLACE_EXISTING);
        }
        // Consider using a ByteArrayOutputStream, which is backed by memory, to save a
        // file rename operation. However,
        // OutputFileWriter is used in many places.
        Files.move(oFile, oFile.resolveSibling(Parameters.LaTeXOutputFile + ".tex"),
            StandardCopyOption.REPLACE_EXISTING);
      } catch (Exception e) {
