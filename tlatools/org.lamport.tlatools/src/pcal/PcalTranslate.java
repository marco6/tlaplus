...
public class PcalTranslate {
    ...
    private static AST.Uniprocess ExplodeUniprocess(AST.Uniprocess ast) throws PcalTranslateException {
        ...
        while (i < ast.prcds.size()) {
            newast.prcds.addElement(
                    ExplodeProcedure((AST.Procedure) ast.prcds.elementAt(i)));
            i = i + 1;
        }
        i = 0;
        newast.body = new Vector(ast.body.size(), 10);
        AST.LabeledStmt thisLS = (ast.body.size() > 0)
                ? (AST.LabeledStmt) ast.body.elementAt(0)
                : null;
        AST.LabeledStmt nextLS = (ast.body.size() > 1)
                ? (AST.LabeledStmt) ast.body.elementAt(1)
                : null;
        while (i < ast.body.size()) {
            ...
            nextLS = (ast.body.size() > i + 1)
                    ? (AST.LabeledStmt) ast.body.elementAt(i + 1)
                    : null;
        }
        return newast;
    }
    ...
    private static AST.Multiprocess ExplodeMultiprocess(AST.Multiprocess ast) throws PcalTranslateException {
        ...
        newast.setOrigin(ast.getOrigin());
        while (i < ast.prcds.size()) {
            newast.prcds.addElement(ExplodeProcedure((AST.Procedure) ast.prcds.elementAt(i)));
            i = i + 1;
        }
        i = 0;
        newast.procs = new Vector(ast.procs.size(), 10);
        while (i < ast.procs.size()) {
            newast.procs.addElement(ExplodeProcess((AST.Process) ast.procs.elementAt(i)));
            i = i + 1;
        }
        return newast;
    }
    ...
    private static Vector CopyAndExplodeLastStmt(Vector stmts, String next) throws PcalTranslateException {
        ...
        if (stmts != null && stmts.size() > 0) {
            ...
            // EitherObj added by LL on 25 Jan 2006
                    ...
                    if (!needsGoto) {
                        ...
                        for (int i = 0; i < Either.ors.size(); i++) {
                            if (((BoolObj) needsGotoVec.elementAt(i)).val) {
                                ((Vector) Either.ors.elementAt(i)).addElement(
                                        UpdatePC(next));
                            }
                        }
                    }
                ...
                needsGoto = true; // result1.addElement(UpdatePC(next));
        } else {
    ...
    private static Vector ExplodeLabeledStmt(AST.LabeledStmt ast,
        ...
        Vector pair = CopyAndExplodeLastStmtWithGoto(new Vector<>(ast.stmts),
                next);
        Vector result = new Vector();
        newast.setOrigin(ast.getOrigin());
        newast.col = ast.col;
    ...
    private static Vector ExplodeLabeledStmtSeq(Vector seq,
        ...
        for (int i = 0; i < seq.size(); i++) {
            ...
            String nxt = (i < seq.size() - 1) ? ((AST.LabeledStmt) seq.elementAt(i + 1)).label : next;
    ...
    private static Vector ExplodeWhile(AST.LabeledStmt ast,
        ...
        PCalLocation whileBeginUnlabDoLoc = (w.unlabDo.size() != 0)
                ? ((AST) w.unlabDo.elementAt(0)).getOrigin().getBegin()
                : w.test.getOrigin().getEnd();
        PCalLocation whileEndUnlabDoLoc = (w.unlabDo.size() != 0)
        ...
        AST.LabeledStmt firstLS = (w.labDo.size() == 0)
                ? null
                : (AST.LabeledStmt) w.labDo.elementAt(0);
        /* explode unlabDo */
        String unlabDoNext = (firstLS == null) ? ast.label : firstLS.label;
        Vector pair1 = CopyAndExplodeLastStmtWithGoto(new Vector<>(w.unlabDo),
                unlabDoNext);
        ...
        for (int i = 0; i < w.labDo.size(); i++) {
            AST.LabeledStmt nextLS = (w.labDo.size() > i + 1)
                    ? (AST.LabeledStmt) w.labDo.elementAt(i + 1)
                    : null;
    ...
    private static Vector ExplodeLabelIf(AST.LabelIf ast, String next) throws PcalTranslateException {
        ...
        AST.LabeledStmt firstThen = (ast.labThen.size() > 0)
                ? (AST.LabeledStmt) ast.labThen.elementAt(0)
                : null;
        AST.LabeledStmt firstElse = (ast.labElse.size() > 0)
                ? (AST.LabeledStmt) ast.labElse.elementAt(0)
                : null;
        Vector explodedThen = CopyAndExplodeLastStmtWithGoto(ast.unlabThen,
                (firstThen == null)
                        ? next
                        : firstThen.label);
        Vector explodedElse = CopyAndExplodeLastStmtWithGoto(ast.unlabElse,
                (firstElse == null)
                        ? next
                        : firstElse.label);
        ...
        while (i < ast.labThen.size()) {
            ...
            nextThen = (ast.labThen.size() > i + 1)
                    ? (AST.LabeledStmt) ast.labThen.elementAt(i + 1)
                    : null;
        }
        /* Explode the labeled else statements */
        AST.LabeledStmt nextElse = (ast.labElse.size() > 1)
                ? (AST.LabeledStmt) ast.labElse.elementAt(1)
                : null;
        i = 0;
        while (i < ast.labElse.size()) {
            ...
            nextElse = (ast.labElse.size() > i + 1)
                    ? (AST.LabeledStmt) ast.labElse.elementAt(i + 1)
                    : null;
        }
    ...
    private static Vector ExplodeLabelEither(AST.LabelEither ast,
        ...
        for (int i = 0; i < ast.clauses.size(); i++) {
            ...
            Vector res = CopyAndExplodeLastStmtWithGoto(
                    clause.unlabOr,
                    (clause.labOr.size() > 0) ? ((AST.LabeledStmt) clause.labOr.elementAt(0)).label : next);
            /**
    ...
    private static Vector ExplodeCall(AST.Call ast, String next) throws PcalTranslateException {
        ...
        ;
        PcalSymTab.ProcedureEntry pe = (PcalSymTab.ProcedureEntry) st.procs.elementAt(to);
        /*******************************************************************
         ...
         * subobjects that set the parameters to that parameter's declaration as a
         * parameter
         * of the procedure. (This code assumes that the declarations appear in the
         * params
    ...
    private static Vector ExplodeCallReturn(AST.CallReturn ast, String next) throws PcalTranslateException {
        ...
        ;
        int from = st.FindProc(ast.from);
        PcalSymTab.ProcedureEntry peFrom = (PcalSymTab.ProcedureEntry) st.procs.elementAt(from);
        int to = st.FindProc(ast.to);
        /*******************************************************************
        ...
        ;
        PcalSymTab.ProcedureEntry peTo = (PcalSymTab.ProcedureEntry) st.procs.elementAt(to);
        ...
        if (!ast.from.equals(ast.to)) {
            for (int i = 0; i < peFrom.decls.size(); i++) {
                AST.PVarDecl decl = (AST.PVarDecl) peFrom.decls.elementAt(i);
                sass = new AST.SingleAssign();
                sass.line = ast.line;
                sass.col = ast.col;
                expr = new TLAExpr();
            ...
            for (int i = 0; i < peTo.decls.size(); i++) {
                AST.PVarDecl decl = (AST.PVarDecl) peTo.decls.elementAt(i);
                expr.addToken(BuiltInToken(","));
                expr.addLine();
                expr.addToken(IdentToken(decl.var));
                expr.addToken(BuiltInToken("|->"));
            ...
            for (int i = 0; i < peTo.params.size(); i++) {
                AST.PVarDecl decl = (AST.PVarDecl) peTo.params.elementAt(i);
                expr.addToken(BuiltInToken(","));
                expr.addLine();
                expr.addToken(IdentToken(decl.var));
                expr.addToken(BuiltInToken("|->"));
