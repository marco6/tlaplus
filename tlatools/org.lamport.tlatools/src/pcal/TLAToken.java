 ...
 *************************************************************************/
{
  ...
  /**
   * The beginSubst and endSubst fields represent sequences of Paren objects in a
   * TPMap,
   ...
   * 
   * Note that there's no need for the TPMap to have the extra Parens
   * corresponding
   * to the location of "b". It wouldn't be needed if substituting an expression
   * containing
   * more than one token for a token always adds parentheses around the
   * substituted
   * expression. This would imply that if some token has beginSubst equal to a
   * sequence
   ...
   * sequence, which is what is done. However, since the
   * {@link TLAExpr#substituteAt}
   * method is sometimes called with an argument telling it not to add
   * parentheses when substituting multiple tokens, I'm letting beginSubst and
   * endSubst
   * be sequences (Vectors) of PCalLocation objects. It would be a good idea to
   * remove redundant pairs of matching parens (the ones for "b" in the example),
   * and
