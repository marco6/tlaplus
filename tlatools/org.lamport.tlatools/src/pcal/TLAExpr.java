...
public class TLAExpr {
    ...
    while (i < tokens.size()) {
      ...
      if (curLine.size() > 0) {
        ...
        while ((j >= 0) && lineNotFound) {
          ...
          if ((ancLine.size() > 0)
              && (((TLAToken) ancLine
                  .elementAt(0)).column <= curLineFirstCol)) { /***************************************************
            ...
            while ((k + 1 < ancLine.size())
                && (((TLAToken) ancLine.elementAt(k + 1)).column <= curLineFirstCol)) {
              k = k + 1;
            }
            ;
  ...
  public void substituteForAll(Vector exprs, // of TLAExpr
      Vector strs, // of String
      boolean parenthesize) throws TLAExprException {
  ...
  { /*******************************************************************
    ...
    if ((expr.tokens.size() == 1)
        && (((Vector) expr.tokens.elementAt(0))
            .size() == 1)) { /***************************************************************
    ...
    while (i < incr) {
      ...
      if (line.size() == result.two) {
        ...
        while ((result.one < tokens.size())
            && (((Vector) tokens.elementAt(result.one)).size() == 0)) {
          result.one = result.one + 1;
        }
        ;
