 ...
 * to regions of the PlusCal code. It is used to implement a command that allows
 * the
 * user to jump from a selected region in the TLA+ translation to the region of
 * the
 * PlusCal code that produced that region of the translation, as well as
 * commands
 * to jump directly from the location indicated in an error report from SANY or
 * TLC
 * to the PlusCal code responsible for the error.
 * <p>
 * The TLA+ to PlusCal mapping is computed and put in the
 * {@link TLAtoPCalMapping#mapping}
 * field of this class. A TLA+ spec of how this mapping is used to map from a
 * region of
 * the translation to a region of the code is contained in the TLAToPCal module
 * that is
 * appended to the end of this file, and that is kept in the generals/docs
 * project.
 * <p>
 * BUGS IN THE IMPLEMENTATION OF THE TRANSLATION MAPPING
 * - Sometimes, the selected region of of the translation should consist of an
 * entire
 ...
 * The current implementation does not use MappingObject.Break objects, so it
 * should
 * map the selected region to the smallest connected region of PlusCal code
 * containing
...
public class TLAtoPCalMapping implements Serializable {
     ...
     * 
     * @param tlaRegion
     *                             The TLA {@link Region} for which the PCal
     *                             {@link Region} is to
     *                             be returned
     * @param currentAlgorithmLine
     *                             The current line of the "--algorithm" or "--fair"
     *                             keyword in
     ...
     * "--algorithm" or "--fair algorithm" token(s) that begin(s) a PlusCal
     * algorithm.
    ...
    /**
     * Returns the PCal code location to which `mapping' maps the tpregion Region in
     * the
     * TLA+ translation, where line numbers in `selection' are relative to
     * tlaStartLine.
    ...
    public static Region ApplyMapping(TLAtoPCalMapping mapping, Region tpregion) {
         ...
         * without it.
         */
        // && ((ltok.getColumn() != rtok.getColumn()) || (ltok.getLine() !=
        // rtok.getLine()))
        ) {
            minDepth++;
        }
        ...
        /*
         * The algorithm of TLAToPCal now looks for breaks between bParen and eParen to
         * compute the
         * sequence of disjoint regions that should really be highlighted. This is not
         * being
         * implemented now, so we skip that part and just construct and return the
         * result.
         */

        /*
         * Set lpos and rpos to the PCalLocations marking the left and right of bParen
         * and eParen.
         */
        PCalLocation lpos;
     ...
     * the left edge of reg is at the end of a token, that token is part of the
     * returned
     * region. This is counterintuitive, and perhaps it should be changed. However,
     * I
    ...
    private static Region RegionToTokPair(MappingObject[][] spec, Region reg) {
        ...
        while (notDone && loc != null) {
            if (prevIsBeginToLeft) {
                MappingObject.EndTLAToken mobj = (MappingObject.EndTLAToken) ObjectAt(loc, spec);
        ...
        while (notDone && loc != null) {
            if (prevIsEndToRight) {
                MappingObject.BeginTLAToken mobj = (MappingObject.BeginTLAToken) ObjectAt(loc, spec);
                // This will throw an exception if a BeginTLAToken is not
                // immediately followed by an EndTLAToken
     ...
     * redundant if it comes immediately inside (with no intervening tokens) a
     * matching
            ...
            while (j < inLine.size()) {
                ...
                if (inObj.getType() == MappingObject.LEFT_PAREN) {
                    ...
                    if (unmatchedLeft.size() != 0) {
                        lastUnmatchedLeft = unmatchedLeft.elementAt(unmatchedLeft.size() - 1);
                    }
                    ...
                    lastMatchedLeft = unmatchedLeft.remove(unmatchedLeft.size() - 1);
     ...
     * both non-null and locA and locB represent consecutive (line, column)
     * locations
 ...
 * tpRegion_1 == Reg(5,20)
 * 
 * tpMap_2 == <<L(10), T(1,2), L(11), T(3,4), L(12), T(5,6), L(13), T(7,8),
 * R(14),
 ...
 * 
 * vars == << tpregion, ltok, rtok, rtokDepth, minDepth, allExpr, bParen,
 * eParen,
 * result, curBegin, lastRparen, i, curDepth, pc >>
 * 
 ...
 * /\ pc' = "Lbl_2"
 * /\ UNCHANGED curDepth
 * ELSE /\ Assert(/\ ParenDepth(tpMap, rtok) = ParenDepth(tpMap, ltok) +
 * rtokDepth
 * /\ minDepth + ParenDepth(tpMap, ltok) =
 * Min({ParenDepth(tpMap, k) : k \in ltok..rtok})
 * /\ allExpr = \A k \in ltok..rtok :
 ...
 * 
 * out = << >>, \* The output
 * unmatchedLeft = << >>, \* Sequence of indices in `out' of unmatched "("s in
 * out
