 ...
 * <br>
 * {@link PcalTLAGen#generate(AST, PcalSymTab)} returns a vector of Strings, one
 * entry per line of generated TLA+.
 * 
 * @version $Id$
...
public class PcalTLAGen {
     ...
     * selfIsSelf is set to true when self is set to "self", and to false when self
     * is
    ...
    private Vector pcV = new Vector(); /*
                                        * sublist of vars of variables representing
     ...
     * or "Next" for a uniprocess algorithm. When ParseAlgorithm.omitPC is true,
     * this is used
     * instead of the label when generating the process's or the entire algorithm's
     * next-state
    ...
    private void GenProcedure(AST.Procedure ast, String context) throws PcalTLAGenException {
        ...
        // The previous version was very convoluted just to avoid having to go through
        // the
        ...
        //
        // int nsC = ast.name.length() + ((mp) ? "(self)".length() : 0) + " ==
        // ".length();
        // if (mp)
        // {
        // self = selfAsExpr(); // subscript for variables is "self"
        ...
        // {
        // AST.LabeledStmt stmt = (AST.LabeledStmt) ast.body.elementAt(i);
        // if ((ns.length() + stmt.label.length() + " \\/ ".length() + ((mp) ?
        // "(self)".length() : 0)) > wrapColumn
        // - nsC - " \\/ ".length())
        // {
    ...
    private void GenProcess(AST.Process ast, String context) throws PcalTLAGenException {
        ...
        // }
        //
        // int nsC = ast.name.length() + ((isSet) ? "(self)".length() : 0) + " ==
        // ".length();
        // if (isSet)
        // {
        // nextStepSelf.addElement(ast.name + "(self)");
        ...
        // {
        // AST.LabeledStmt stmt = (AST.LabeledStmt) ast.body.elementAt(i);
        // if ((ns.length() + stmt.label.length() + " \\/ ".length() + ((isSet) ?
        // "(self)".length() : 0)) > wrapColumn
        // - nsC - " \\/ ".length())
        // {
        // nsV.addElement(ns.toString());
    ...
    private void GenLabeledStmt(AST.LabeledStmt ast, String context) throws PcalTLAGenException {
         ...
         * statement
         * that came from the PCal code arose from the expansion of a macro call, then
         * we
     ...
     * numbers starting with startCol are decreased by `shift'. If any
     * Begin/EndTLAToken
            ...
            for (int i = 0; i < procs.size(); i++) {
                ...
                for (int p = 0; p < proc.decls.size(); p++) {
                    ...
                    if (!mp) {
                        ...
                        // if (mp)
                        // {
                        // sv = AddSubscriptsToExpr(decl.val, SubExpr(Self("procedure")), new
                        // Changed(new Vector()))
                        // .toStringVector();
                        // } else
                        // {
        ...
        if (processes != null && processes.size() > 0) {
            ...
            for (int i = 0; i < processes.size(); i++) {
                ...
                for (int p = 0; p < proc.decls.size(); p++) {
                    ...
                    // everything from here down to the end of the for p loop should be commented
                    // out
                    // is.append(decl.var);
                    // is = new StringBuffer(decl.var);
                    // if (decl.isEq)
                    ...
                    // * "[self]". *
                    // ***********************************************/
                    // sve = AddSubscriptsToExpr(decl.val, SubExpr(Self("procedure")), new
                    // Changed(new Vector()));
                    // } else
                    // {
                    // /************************************************
            ...
            for (int i = 0; i < st.processes.size(); i++) {
                ...
                if (fairness != AST.UNFAIR_PROC) {
                    ...
                    for (int k = 0; k < procedures.size(); k++) {
                        ...
                        PcalSymTab.ProcedureEntry pe = (PcalSymTab.ProcedureEntry) st.procs.elementAt(procedureIndex);
        ...
        for (int i = 0; i < expr.tokens.size(); i++) {
            ...
            for (int j = 0; j < tv.size(); j++) {
                ...
                boolean subr = (sub != null && (tok.type == TLAToken.ADDED || (mp && (tok.type == TLAToken.IDENT)
                        && (IsProcedureVar(tok.string) || IsProcessSetVar(tok.string)))));
