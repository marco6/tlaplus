...
/**
 * This is the main entry point for the TLA+ front end that performa parsing,
 * semantic analysis,
...
public class SANY {
   ...
   * However, if it is zero, then you should also check
   * {@link SpecObj#getParseErrors()}
   * and {@link SpecObj#getSemanticErrors()} by calling the
   * {@link Errors#isSuccess()}
  ...
  public static final SanyExitCode parse(
    ...
    try {
      ...
      if (settings.doSemanticAnalysis) {
        ...
        // **** Linting
        if (settings.doLevelChecking && SanyExitCode.OK == SanyExitCode.fromCode(spec.getErrorLevel())
            && settings.doLinting) {
  ...
  /**
   * Reports the messages to the output. Returns true if any elevated messages
   * were encountered.
   */
  ...
  /**
   * Use {@link SANY#frontEndParse(SpecObj, SanyOutput, SanySettings)} instead for
   * greater
   * control of output.
   */
  @Deprecated
  ...
  public static void frontEndParse(SpecObj spec, SanyOutput out, SanySettings settings)
    ...
    try {
      ...
      // get here if either the TLAPlusParser.parse() threw a ParseException or
      // spec.ParseErrors was not empty
      spec.errorLevel = SanyExitCode.SYNTAX_PARSING_FAILURE.code();
      throw new ParseException();
    } catch (Exception e) {
      spec.errorLevel = SanyExitCode.SYNTAX_PARSING_FAILURE.code();
    ...
    try {
      ...
      for (int i = 0; i < spec.semanticAnalysisVector.size(); i++) {
        ...
        if (externalModuleTable.getContext(UniqueString.uniqueStringOf(moduleStringName)) == null) {
          ...
          // Perform semantic analysis and create semantic graph for one external module
          // here
  ...
  private static void printUsage() {
    ...
    variant.add(new UsageGenerator.Argument(
        ...
        "Suppress specific messages; comma-separated list of message codes.\n" +
            ...
            "Message codes can be found in https://github.com/tlaplus/tlaplus/blob/master/tlatools/org.lamport.tlatools/src/tla2sany/semantic/ErrorCode.java#L38",
        true));
    variant.add(new UsageGenerator.Argument(
  ...
  public static void SANYmain0(String args[]) throws SANYExitException {
    ...
    // set of message codes to elevate to errors; populated by -messagesAsErrors CLI
    // flag
    ...
    // Check that suppressMessages and messagesAsErrors do not intersect and report
    // an error if they do.
    Set<ErrorCode> intersection = new HashSet<>(suppressedCodes);
    intersection.retainAll(messagesAsErrorCodes);
    if (!intersection.isEmpty()) {
      ToolIO.out.println(
          "Error: The following codes were set to both -suppressMessages and -messagesAsErrors: " + intersection);
