...
public class OperatorStack implements tla2sany.st.SyntaxTreeConstants {
  ...
  /*
   * We use the prec and succ relations, as described in the formal specs of the
   * grammar.
  ...
  final public boolean preInEmptyTop() {
     ...
     ***********************************************************************/
    if (CurrentTop == null)
      return true; // empty or holding nothing - lookahead compliant
              ...
              if (tm2.isOperator()) {
                ...
                if (Operator.succ(oL, oR)
                  ...
                  if (oL.isInfix())
                    ...
                    if ((tm2.getNode().getLocation().beginLine() < tm1.getNode().getLocation().beginLine())
  ...
  final public SyntaxTreeNode finalReduce() throws ParseException {
    ...
    if (isWellReduced())
      ...
      int l[];
      do {
        // (CurrentTop.elementAt(n)).getNode().printTree(new
        // java.io.PrintWriter(System.out)); n++;
