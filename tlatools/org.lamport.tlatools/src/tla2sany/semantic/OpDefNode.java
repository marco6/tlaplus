...
public class OpDefNode extends OpDefOrDeclNode
    ...
    if (this.getKind() == ModuleInstanceKind) {
      ...
      if (args == null | params == null) { // args vector should never be null
        ...
        if (params.length != args.length) {
          ...
          // if the operator is a built-in op... (We separate out the logic for the
          // builtin ops
          // because there are no FormalParamNodes in the semantic tree to describe their
          // arguments
          if (this.getKind() == BuiltInKind) {
            ...
            for (int i = 0; i < params.length; i++) {
              // if i'th FormalParamNode shows arity == 0, then an expression is expected as
              // argument
   ...
   ***********************************************************************/
  /**
   * This "getters" for isLeibnizArg and isLeibniz were added by LL on 24 Oct
   * 2012.
      ...
      case UserDefinedOpKind:
        ...
        if (params != null) {
          ...
          for (int i = 0; i < params.length; i++) {
            ...
            if (isLeibnizArg != null && isLeibnizArg[i])
