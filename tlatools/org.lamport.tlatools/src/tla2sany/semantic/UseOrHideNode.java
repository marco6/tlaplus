...
public class UseOrHideNode extends LevelNode {
  ...
  public void factCheck(Errors errors) {
    ...
    for (int i = 0; i < this.facts.length; i++) {
      if ((this.facts[i].getKind() == OpApplKind)
          && (((OpApplNode) this.facts[i]).operator.getKind() != ThmOrAssumpDefKind)) {
        errors.addError(
  ...
  protected Element getLevelElement(Document doc, SymbolContext context,
    ...
    // SemanticNode.SymbolContext context = new
    // SemanticNode.SymbolContext(context2);
    ...
    for (int i = 0; i < facts.length; i++)
      factse.appendChild(facts[i].export(doc, context, filter));
    for (int i = 0; i < defs.length; i++)
      definitions.appendChild(defs[i].export(doc, context, filter));
