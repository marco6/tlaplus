...
public class ModulePointer {
  ...
  // Returns reference to the N_Extends TreeNode heading the EXTENDS declaration
  // (even if empty)
  TreeNode getExtendsDecl() {
    return treeNode.heirs()[1];
  }

  // Returns reference to the N_Body TreeNode heading the body of the module,
  // (even if empty)
  TreeNode getBody() {
    return treeNode.heirs()[2];
  }

  // Returns a triple of the file name, the module name, and--to disambiguate
  // among
