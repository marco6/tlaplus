...
public class ParseUnit {
  ...
  final Vector<ParseUnit> getExtendedBy() {
    return parseUnitRelatives.extendedBy;
  }
  ...
  final Vector<ParseUnit> getInstancedBy() {
    return parseUnitRelatives.instancedBy;
  }

  // Add-methods
  final void addExtendee(ParseUnit pu) {
    parseUnitRelatives.extendees.addElement(pu);
  }

  final void addExtendedBy(ParseUnit pu) {
    parseUnitRelatives.extendedBy.addElement(pu);
  }

  final void addInstancee(ParseUnit pu) {
    parseUnitRelatives.instancees.addElement(pu);
  }

  final void addInstancedBy(ParseUnit pu) {
    parseUnitRelatives.instancedBy.addElement(pu);
  }
   ...
   */
  public final void parseFile(Errors errors, boolean firstCall, String name, ParseUnit rootParseUnit, SanyOutput out)
      throws AbortException {
    // Has it already been parsed since last modified? If yes, then no need to parse
    // again
    if (parseStamp > nis.sourceFile().lastModified())
      return;

    // Does the file exist? If not abort cleanly. Of course the file could be
    // deleted
    // during the next few lines, and execution would also abort, but not so
    // cleanly.
    ...
    if (ToolIO.getMode() == ToolIO.SYSTEM) {
      // If `resolver.getLibraryPath` returns `null` or the file is not an instance of
      // TLAFile
      // (and as we didn't have errors in previous steps, the module
      // really exists somewhere), then we assume that this file is part of a monolith
      // file.
  ...
  private void handleExtensions(ModulePointer currentModule, ModulePointer otherModule) {
     ...
     * System.err.println("Leaving HandleExtensions for " + currentModule.getName()
     * + "; context is:");
     * System.err.println(currentModule.getContext().toString());
     * }
     */
  ...
  private void calculateContextWithinParseUnit(ModulePointer currentModule) {
    // System.err.println("Context before adding parents: " +
