...
/**
 * Originally i was attempting to run the model check in the same JVM in which
 * {@link TraceExplorer} was invoked (and
 * then did its parsing and generating TLA and CFG files.) The model check
 * process would fail with a message of:
 * 
 * @!@!@STARTMSG 2233:1 @!@!@
 * The initial predicate _TEInit cannot be a constant.
 * @!@!@ENDMSG 2233 @!@!@
 * 
 * ... however, running TLC from command line freshly on the same TLA/CFG would
 * not produce this error. I can only
 * surmise that the parsing process is setting some static value somewhere which
 * is not being cleared and which is
 * then making the model checking process fail.
 * 
 * In light of that, i've written this class to spin off a new JVM for the model
 * check.
 */
public class TLCRunner {
	...
	/**
	 * @param flag if true, no output from the TLC process will be sent to
	 *             System.out
	 */
