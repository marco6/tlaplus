 ...
 *
 * <p>
 * TLC uses LazyValue when it "computes" the arguments to an operator or the
 * bound value in a LET...IN expression.
 * Because TLA+ has temporal operators, it cannot behave like a normal
 * call-by-value interpreter in these cases.
 *
 * <p>
 * For example, suppose a TLA+ spec contains the definition
 * 
 * <pre>
 *     F(x) == x'
 * </pre>
 * 
 * and TLC has to evaluate the expression <code>F(var)</code> in a behavior
 * where <code>var = 1</code> and
 * <code>var' = 2</code>. The correct output is 2. However, fully reducing the
 * argument <code>var</code> before
 ...
 * </pre>
 * 
 * When expanding <code>F</code> TLC needs to keep the expression that was used
 * as the <code>x</code> argument in order
 * to get the right output.
 *
 * <h2>Caching and Performance</h2>
 *
 * <p>
 * For performance, LazyValue can cache the reduced value when it is computed.
 * This enables TLC to have similar
 * performance to a call-by-value language in many cases. For example, if
 * <code>G(x) == x + x</code>, then evaluating
 * the expression <code>G(very_expensive_arg)</code> should only evaluate the
 * very expensive argument once.
 *
 * <p>
 * TLC has had multiple soundness bugs due to mishandling of cached LazyValues:
 * see Github issues #113 and #798. As
 * a result, reads of the cached value are now guarded and callers must specify
 * the tool, behavior, and evaluation mode
 * they are using when reading it (see
 * {@link #getCachedValue(Tool, TLCState, TLCState, int)}).
 *
 * <p>
...
public class LazyValue extends Value {
  ...
  public LazyValue(SemanticNode expr, Context con, final boolean cachable, final CostModel cm) {
    ...
    // See other note about a bug that surfaced with LazyValue in the docstring
    // above.
