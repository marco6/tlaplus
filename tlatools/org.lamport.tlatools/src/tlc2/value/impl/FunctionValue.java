 ...
 *
 * <p>
 * TODO: move some of the function operations from {@link Value} to this
 * interface,
 * such as {@link Value#takeExcept(ValueExcept)} and
...
public interface FunctionValue {
   ...
   *
   * <p>
   * All functions in TLA+ are unary. When a function is applied to multiple
   * arguments
   * as in <code>f[1, 2]</code>, it is just syntactic sugar for applying the
   * function to a
   * tuple of those arguments (e.g. <code>f[&lt;&lt;1, 2&gt;&gt;]</code>).
   * Therefore, the
   * default implementation of this method calls {@link #apply(Value, int)} with
   * <code>args</code>
   * wrapped in a {@link TupleValue}. If <code>args.length == 1</code>, it does
   * not create
   ...
   * @throws util.Assert.TLCRuntimeException if the argument is not in the
   *                                         function's domain
   */
   ...
   *
   * <p>
   * This method is very similar to {@link #apply(Value, int)} but with two
   * critical
   * differences:
   * <ol>
   * <li>Evaluation control is assumed to be {@link EvalControl#Clear}</li>
   * <li>Returns null instead of throwing an exception when the argument is not in
   * the function's domain</li>
   * </ol>
   *
   * @param arg the argument
   * @return the apply result or null if the argument is not in this function's
   *         domain
   * @throws EvalException if evaluation fails
   */
