...
public class FcnRcdValue extends Value implements FunctionValue, IFcnRcdValue {
  ...
  // This implementation has a concurrency bug:
  ...
  final Value selectBinarySearch(final Value arg) {
    ...
    if (this.isNorm && this.domain.length >= LINEAR_SEARCH_THRESHOLD) {
      ...
      if (idx >= 0 && this.domain[idx].equals(arg)) {
        // Check equality and cmp here to not introduce subtle bugs should
        // Value#compareTo
  ...
  /**
   * {@link #size()} first normalizes, destructively, this instance; for
   * inspections on the size without normalization
   * use this method.
   * 
   * @return
   */
  ...
  public final IValue deepCopy() {
    try {
      ...
      // WRT Arrays.copyOf, see comment in tlc2.value.impl.RecordValue.deepCopy()
      // introduced
