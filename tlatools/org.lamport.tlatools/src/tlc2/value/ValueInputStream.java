...
public final class ValueInputStream implements ValueConstants, IValueInputStream {
	...
	/**
	 * This used to be readExternal, but the tbl parameter did not end up being
	 * useful.
	 * It was meant to allow using a different string intern table, but as a result
	 * required all deserialized string values to be known ahead of time (in the
	 * map).
	 * 
	 * This stub calls readExternal, ignoring tbl and directly loading new strings
	 * into
	 ...
	 * 
	 * TODO:
	 * - Once all practically deployed TLCs that don't have readExternal are phased
	 * out,
	 * change community modules to call readExternal.
	 * - Once all practically deployed community modules that use this method are
	 * phased out,
	 * delete it.
	 */
	@Deprecated
	...
	/**
	 * Safely read a value from an external source that may have a different intern
	 * table for UniqueStrings.
	 * By contrast, {@link #read()} assumes that the value was written by this
	 * process using the same intern table.
	 */
