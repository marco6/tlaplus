...
/**
 * There were a lot of utility methods in the toolbox's ModelWriter class that
 * made what statefulness of that class which
 * existed, obscured. I broke out the utility methods into their own class on
...
public final class SpecWriterUtilities {
    ...
    /**
     * A pattern to match IDs generated by the
     * {@link AbstractSpecWriter#getValidIdentifier(String)} method
     */
    public static final Pattern ID_MATCHER = Pattern.compile("(" + TLAConstants.Schemes.SPEC_SCHEME + "|"
     ...
     * 
     * @param scheme a naming scheme, one of the {@link TLAConstants.Schemes}
     *               constants
     ...
     * This was created to not mess with {@link getValidIdentifier} which is used by
     * other classes.
     * 
     * @param scheme a naming scheme, one of the {@link TLAConstants.Schemes}
     *               constants
    ...
    public static List<String[]> createListContent(final List<Formula> formulaList, final String labelingScheme) {
        ...
        for (int i = 0; i < formulaList.size(); i++) {
            ...
            content = new String[] { label, label + TLAConstants.DEFINES_CR + formulaList.get(i).getFormula(),
                    String.valueOf(i) };
     ...
     *                       if true, the method returns module name, iff the
     *                       specified file exists or null, if the specified file
     *                       does not exist, if false - only string manipulations
     *                       are executed
     * @return module name
     */
     ...
     */
    public static StringBuilder getExtendingModuleContent(final String moduleFilename,
     ...
     * Converts formula list to a string representation
     * 
     * @param serializedFormulaList, list of strings representing formulas (with
     *                               enablement flag)
     * @param labelingScheme         one of the {@link TLAConstants.Schemes}
     *                               constants
     * @return
     */
    ...
    /**
     * @param value                      if null or empty, an empty list will be
     *                                   returned
    ...
    /**
     * Create a list of overrides. If the override is not in the spec's root module,
     * then
     * the config file will have A <- [M] id . This means that A is defined in
     * module M,
     * and its definition is being overriden in the spec root module which is
     * dependent upon M.
     * The following is an example from Leslie Lamport that explains what occurred
     * before changing
            ...
            if (defNode == null) {
                ...
                if (source == defNode) {
                    ...
                    if (formula.isModelValue() && !formula.isSetOfModelValues()) {
                        // model value
                        content = new String[] { formula.getLabel() + TLAConstants.EQ + formula.getLabel(),
                    ...
                    if (formula.isModelValue() && !formula.isSetOfModelValues()) {
                        ...
                        content = new String[] {
                                source.getName().toString() + TLAConstants.ARROW + "["
                                        ...
                                        + " " + id + TLAConstants.EQ + source.getName().toString(),
                                "CONSTANT " + id };
                    } else {
                        // not a model value
                        content = new String[] {
