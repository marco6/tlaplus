 ...
 * strings inside of the code. Any kind of String used as a message reported to
 * the user should be replaced with
 ...
 * <br>
 * <br>
 * Here is an example for usage of non-parameterized messages. Assume we have
 * code like this:
 * <code>Assert.fail("getLevel called for TheoremNode before levelCheck");</code>
 * which should be replaced by the call of Assert with the error code. First of
 * all we need to add an additional
 * case in the method {@link MP#getMessage(int, int, String[])}. For doing this,
 * a new constant is required.
 ...
 * <br>
 * <br>
 * After that, we replace the code Assert.fail("getLevel called for TheoremNode
 * before levelCheck"); by
 * <code>Assert.fail(EC.GET_LEVEL_THEOREM_BEFORE_LEVEL_CHECK)</code>; There will
 * be a warning inside of the
 * class, that SANYCodes can not be resolve, so we add it to the imported
 * classes. In order to do this quickly,
 * put the cursor into the line with the error, and hit <key>Ctrl + 1</key>. The
 * opened quick assist will propose to add the import...
 * Another acceleration can be achieved by adding the case first and pointing
 * the non-existent constant. Again the error
 * will appear in the MessageResolver class and the <key>Ctrl + 1</key> will
 * open the quick assist first proposition will be to add
 * the constant to the {@link EC}. This saves the typing of the name of the
 * constant and the modifier, but you have to put
 ...
 * <br>
 * <br>
 * The message is actually the same, but instead of TheoremNode it has
 * LabelNode. In order to keep with it you can add another
 * constant, or re-think the semantics of the actual message. So in this case I
 * would change the semantics and rename the
 ...
 * <br>
 * <br>
 * Please note the %1% in the string. This will be replaced by the first
 * parameter. The call will be:
 * <code>Assert.fail(EC.GET_LEVEL_BEFORE_LEVEL_CHECK, new String[]{"TheoremNode"});</code>
 * and
 * <code>Assert.fail(EC.GET_LEVEL_BEFORE_LEVEL_CHECK, new String[]{"LabelNode"});</code>
 * respectively.
 * <br>
 * <br>
 * There is also a shortcut for writing this:
 * <code>Assert.fail(EC.GET_LEVEL_BEFORE_LEVEL_CHECK, "LabelNode");</code> which
 * is a shortcut.
 * <br>
 * <br>
 * In general, you can insert more %i% into the message and provide the
 * parameters in the String array.
 * <br>
 * <br>
 * 
...
public class MP {
    ...
    private static final ThreadLocal<DecimalFormat> DF = ThreadLocal
        ...
        switch (messageCode) {
            ...
            case EC.SYSTEM_ERROR_CLEANING_POOL:
                if (messageClass == ERROR) {
                    ...
                    b.append(
                            "Failed to clean up an obsolete disk file. Please manually delete %1% if free disk space is low.");
                }
                break;
            ...
            case EC.TLC_FINGERPRINT_EXCEPTION:
                b.append("TLC was unable to fingerprint."
                        ...
                        + "\n%1%"
                        + "\nReason:"
                        + "\n%2%"); // reason must come last, with nothing following, because Tool cannot parse the
                                    // embedded error otherwise
            ...
            case EC.TLC_DISTRIBUTED_WORKER_STATS:
                // new Date() + " Worker: " + name + " Sent: " + sentStates + " Rcvd: " +
                // receivedStates
            ...
            case EC.TLC_DISTRIBUTED_EXCEED_BLOCKSIZE:
                b.append("Trying to limit max block size (to recover from transport failure): %1% (").append(now())
                        .append(")");
                break;
             ...
             * get changed
             * (see
             * org.lamport.tla.toolbox.tool.tlc.output.data.TLCModelLaunchDataProviderTest)!
             * !!
             */
            case EC.TLC_MODE_MC:
                b.append(
                        "Running breadth-first search Model-Checking with fp %13% and seed %12% with %1% worker%2% on %3% cores with %10%MB heap and %11%MB offheap memory");
            ...
            case EC.TLC_MODE_MC_DFS:
                b.append(
                        "Running depth-first search Model-Checking with fp %13% and seed %12% with %1% worker%2% on %3% cores with %10%MB heap and %11%MB offheap memory");
                if (!"".equals(parameters[13])) {
                    b.append(" [pid: %14%]");
                } else {
            ...
            case EC.TLC_MODE_SIMU:
                b.append(
                        "Running %14% Simulation with seed %1% with %2% worker%3% on %4% cores with %11%MB heap and %12%MB offheap memory");
            ...
            case EC.TLC_STATE_GRAPH_OUTDEGREE:
                b.append(
                        "The average outdegree of the complete state graph is %2% (minimum is %1%, the maximum %4% and the 95th percentile is %3%).");
                break;
            ...
            case EC.TLC_PROGRESS_STATS_DFID:
                if (TLCGlobals.tool) {
                    // same format as model checking progress reporting for easier parsing by the
                    // toolbox
            ...
            case EC.TLC_PROGRESS_SIMU:
                if (TLCGlobals.tool) {
                    ...
                    b.append(
                            "Progress: %1% states checked, %2% traces generated (trace length: mean=%3%, var(x)=%4%, sd=%5%)");
                }
                break;

            case EC.TLC_COVERAGE_START:
                b.append("The coverage statistics at " + now()
                        + " (see https://explain.tlapl.us/module-coverage-statistics for how to interpret the following statistics).");
                break;
     ...
     * 
     * @param errorCode
     * @param parameters a list of string parameters to be inserted into the
     ...
     * 
     * All calls of this method have includeStackTrace true. This printing of the
     * stack trace
     * was added in tlc2--probably by SZ. The stack trace provides no useful
     * information to the
     * user, so LL changed this on 20 Mar 2012 so the trace is printed only when TLC
     * is called
    ...
    public static int printTLCRuntimeException(final TLCRuntimeException tre) {
        if (tre.parameters != null) {
            ...
            DebugPrinter
                    .print("entering printTLCRuntimeException(TLCRuntimeException) with errorCode " + tre.errorCode); //$NON-NLS-1$
            // write the output
    ...
    /**
     * Prints a warning (if the global switch is enabled and it is not a duplicate
     * warning)
     * 
     * @param errorCode
     * @param parameters
