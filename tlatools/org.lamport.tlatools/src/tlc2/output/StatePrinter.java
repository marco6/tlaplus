...
public class StatePrinter {
    private static final int STATE_OVERWRITE_INTERVAL = Integer.getInteger(StatePrinter.class.getName() + ".overwrite",
            -1);
    ...
    public static void printRuntimeErrorStateTraceState(TLCState currentState, int num) {
        MP.printState(EC.TLC_STATE_PRINT1, new String[] { String.valueOf(num), currentState.toString() }, currentState,
                num);
    }
    ...
    public static void printInvariantViolationStateTraceState(TLCStateInfo currentStateInfo) {
        if (currentStateInfo.state.isInitial()) {
            ...
            StatePrinter.printInvariantViolationStateTraceState(currentStateInfo, null,
                    (int) currentStateInfo.stateNumber);
        } else {
            StatePrinter.printInvariantViolationStateTraceState(currentStateInfo,
                    currentStateInfo.state.getPredecessor(), (int) currentStateInfo.stateNumber);
        }
    }
     ...
     * @param num              The index of the state in the trace, counting from
     *                         one.
     */
    public static void printInvariantViolationStateTraceState(TLCStateInfo currentStateInfo, TLCState previousState,
            int num) {
        printInvariantViolationStateTraceState(currentStateInfo, previousState, num, false);
    }

    public static void printInvariantViolationStateTraceState(TLCStateInfo currentStateInfo, TLCState previousState,
            int num, final boolean isFinal) {
        final String stateString = null != previousState && TLCGlobals.printDiffsOnly
                ? currentStateInfo.state.toString(previousState)
                : currentStateInfo.state.toString();

        // Fingerprint can't be calculated when state is incomplete; just use a random
        // value.
        final String fingerprint = currentStateInfo.state.allAssigned()
     ...
     * @param stateNum         The index of the state in the trace, counting from
     *                         one.
     */
    public static void printBackToState(final TLCStateInfo currentStateInfo, final int stateNum) {
        if (TLCGlobals.tool) {
            // TODO If the unit test suite runs with -tool mode always turned on, many tests
            // fail because of a NPE. The NPE results
            // from null passed here as TLCState, which eventually causes the NPE in
            // tlc2.tool.TLCStateInfo.toString(). When I changed
            // this from printState to printMessage, nothing obvious broke. However, I
            // suspect some corner case in the Toolbox breaks,
            // which is why I decided not to touch this.
            MP.printState(EC.TLC_BACK_TO_STATE,
                    new String[] { Integer.toString(stateNum), currentStateInfo.info.toString() }, (TLCState) null,
                    stateNum);
        } else {
            MP.printMessage(EC.TLC_BACK_TO_STATE,
                    new String[] { Integer.toString(stateNum), currentStateInfo.info.toString() });
        }
    }
}
