...
/**
 * Saves all messages containing info about error traces that pass through
 * {@link tlc.output.MP}.
 * Ideally this will eventually go away and all of TLC's model checking
 * implementations will
 * bubble their error traces up through their top-level .run() methods, but
 * until that
 * refactoring takes place this is how we get the error trace: by hooking into
 * the static
 ...
 * - Basic local BFS model checking in
 * {@link tlc2.tool.ModelChecker#doNextCheckInvariants}
 * > note: appears to be dead, the concurrent BFS implementation is always used
 * instead
 * - Concurrent local BFS model checking in
 * {@link tlc2.tool.Worker#doNextCheckInvariants}
 ...
 * - Distributed model checking in
 * {@link tlc2.tool.distributed.TLCServerThread#run}
 * All of these pass through {@link tlc2.output.StatePrinter} then
 * {@link tlc.output.MP}.
 * 
 * The purpose of this class is to record error trace output from all of those
 * sources while
 * ignoring output that is not an error trace (some of which superficially
 * resembles error
