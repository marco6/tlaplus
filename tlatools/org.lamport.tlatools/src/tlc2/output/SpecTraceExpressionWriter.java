...
/**
 * This is a reified class of spec writer which can produce specs capable of
 * containing trace expressions; it is also
 * the parent class for a more specialized version used by the toolbox,
 * {@code TraceExpressionModelWriter}.
 */
public class SpecTraceExpressionWriter extends AbstractSpecWriter {
	 ...
	 * @param cfgBuffer      if non-null, the buffer into which the CFG code will be
	 *                       placed
	 * @param trace
	 * @param expressionData data on trace explorer expressions, can be null
	 * @return String[], first element is the identifier for the initial state
	 *         predicate,
	 *         second element is the identifier for the next-state action, the third
	 *         element is the identifier for
	 *         the action contraint
	 * @see #addInitNextToBuffers(StringBuilder, StringBuilder, List,
	 *      TraceExpressionInformationHolder[], String, String, String)
	 */
	public static String[] addInitNextToBuffers(final StringBuilder tlaBuffer, final StringBuilder cfgBuffer,
		...
		final String actionConstraintId = SpecWriterUtilities
				.getValidIdentifier(TLAConstants.Schemes.ACTIONCONSTRAINT_SCHEME);
	 ...
	 * 
	 * @param tlaBuffer          the buffer into which the TLA code will be placed
	 * @param cfgBuffer          if non-null, the buffer into which the CFG code
	 *                           will be placed
	 * @param trace
	 * @param expressionData     data on trace explorer expressions, can be null
	 * @param initId             the identifier to be used for the initial state
	 *                           predicate, cannot be null
	 * @param nextId             the identifier to be used for the next-state
	 *                           action, cannot be null
	 * @param actionConstraintId the indentified used for the action constraint
	 * @see #addInitNextToBuffers(StringBuilder, StringBuilder, List,
	 *      TraceExpressionInformationHolder[], String, String, String, String)
	 */
	 ...
	 * and then concatenates the returned {@code StringBuilder} instances in order
	 * to tlaBuffer.
	 * 
	 * @param tlaBuffer                    the buffer into which the TLA code will
	 *                                     be placed
	 * @param cfgBuffer                    if non-null, the buffer into which the
	 *                                     CFG code will be placed
	 * @param trace
	 * @param expressionData               data on trace explorer expressions, can
	 *                                     be null
	 * @param initId                       the identifier to be used for the initial
	 *                                     state predicate, cannot be null
	 * @param nextId                       the identifier to be used for the
	 *                                     next-state action, cannot be null
	 * @param actionConstraintId           the identifier used for the action
	 *                                     constraint
	 * @param nextSubActionBasename        the base string to be used as the prefix
	 *                                     to unique names for next sub-actions
	 * @param leaveStubsForTraceExpression if true, then a variable will be defined
	 *                                     {@link TRACE_EXPRESSION_VARIABLE},
	 *                                     yet commented out, and similarly
	 *                                     conjoined, but commented out in the
	 *                                     SpecTE Init and Next
	 *                                     declarations
	 */
	public static void addInitNextToBuffers(final StringBuilder tlaBuffer, final StringBuilder cfgBuffer,
			final List<MCState> trace, final TraceExpressionInformationHolder[] expressionData, final String initId,
	...
	/**
	 * This will set initId equal to the initial state predicate, nextId equal to
	 * the next state
	 * action for the trace, and actionConstraintId equal to the action constraint
	 * for the trace.
	 * If expressionData is not null, it should contain information about trace
	 * explorer expressions. This
	 * information is used to appropriately put the variables representing trace
	 * explorer expressions
	 * in the trace. In the following example, trace explorer expressions are used,
	 * but if expressionData
	 * is null, those variables will not appear in the init and next definitions,
	 * but everything else will be the same.
	 * 
	 * Note: In the following example, the expressions expr1,...,expr6, texpr1,
	 * texpr2 can take up multiple
	 * lines.
	 * 
	 ...
	 * @param nextId                       the identifier to be used for the
	 *                                     next-state action, cannot be null
	 * @param actionConstraintId           the indentified used for the action
	 *                                     constraint
	 * @param nextSubActionBasename        the base string to be used as the prefix
	 *                                     to unique names for next sub-actions
	 * @param leaveStubsForTraceExpression if true, then a variable will be defined
	 ...
	 *                                     SpecTE Init and Next
	 *                                     declarations
	 * @return an array of length 2, the first element is a buffer containing all
	 *         trace expression subaction
	 *         declarations followed by the action constraint definition; the second
	 *         element is a buffer
	 *         containing a potential VARIABLE stub for the trace expression
	...
	public static StringBuilder[] addInitNextToBuffers(final StringBuilder cfgBuffer,
		...
		if (trace.size() > 0) {
			...
			if (cfgBuffer != null) {
				...
				cfgBuffer.append(TLAConstants.COMMENT).append(TLAConstants.KeyWords.ACTION_CONSTRAINT)
			...
			while (nextState != null) {
				 ...
				 * ))
				 */
				subActionsAndConstraint.append(TLAConstants.INDENT).append(TLAConstants.L_PAREN)
						.append(TLAConstants.CR);
				...
				for (int i = 0; i < currentStateVars.length; i++) {
					...
					subActionsAndConstraint
							.append(currentStateVar.getValueAsStringReIndentedAs(TRI_INDENT + TLAConstants.INDENT));
				...
				for (int i = 0; i < currentStateVars.length; i++) {
					...
					subActionsAndConstraint.append(TLAConstants.EQ).append(TLAConstants.L_PAREN)
							.append(TLAConstants.CR);
					subActionsAndConstraint
							.append(nextStateVar.getValueAsStringReIndentedAs(TRI_INDENT + TLAConstants.INDENT));
					subActionsAndConstraint.append(TLAConstants.CR);
					subActionsAndConstraint.append(TRI_INDENT).append(TLAConstants.R_PAREN).append(TLAConstants.CR);
				}
	...
	public static String addTraceFunctionToBuffers(final StringBuilder tlaBuffer, final StringBuilder cfgBuffer,
		...
		if (trace.isEmpty()) {
			return addArrowAssignmentIdToBuffers(tlaBuffer, cfgBuffer,
					new Assignment(TLAConstants.TraceExplore.TRACE, new String[0],
							TLAConstants.BEGIN_TUPLE + TLAConstants.END_TUPLE),
					id, configId);
		}
		...
		for (int j = 0; j < trace.size(); j++) {
			...
			traceFunctionDef.append(TLAConstants.INDENT).append(TLAConstants.L_PAREN).append(state.asSimpleRecord())
	...
	/**
	 * This only changes the tla file. This method generates and adds a variable
	 * declaration
	 ...
	 * It returns an array of {@link TraceExpressionInformationHolder} where each
	 * element
	 * contains the expression, the identifier, and the variable name.
	 * 
	 * If the expressions are x' + y and x > 3, The tla file will contain something
	 * like
	 * 
	 * \* comment line
	 ...
	 * ----
	 * 
	 * @param expressions a list of formulas, each one an expression the user wants
	 *                    to have evaluated
	...
	public TraceExpressionInformationHolder[] createAndAddVariablesAndDefinitions(final List<Formula> expressions,
		...
		final TraceExpressionInformationHolder[] expressionData = TraceExpressionInformationHolder
				.createHolders(expressions, attributeName);
	...
	public void addTraceExpressionStub(String moduleName, String teName, final List<String> variables) {
		...
		// Create local string build for expression and comments so we can indent
		// everything together in the end.
		StringBuilder localBuffer = new StringBuilder();

		localBuffer.append(TLAConstants.COMMENT)
				.append(String.format("To hide variables of the `%s` spec from the error trace,", moduleName))
				.append(TLAConstants.CR);
		localBuffer.append(TLAConstants.COMMENT)
				.append("remove the variables below.  The trace will be written in the order").append(TLAConstants.CR);
		...
		localBuffer.append(
				variables.stream().map(var -> var + TLAConstants.RECORD_ARROW + var).collect(Collectors
						.joining(TLAConstants.CR + TLAConstants.COMMA)))
				.append(TLAConstants.CR).append(TLAConstants.CR);

		localBuffer.append(TLAConstants.COMMENT)
				.append("Put additional constant-, state-, and action-level expressions here:").append(TLAConstants.CR);
		...
		if (variables.size() > 0) {
			...
			localBuffer.append(TLAConstants.COMMENT)
					.append(String.format(",_%sUnchanged |-> %s = %s'", someVar, someVar, someVar))
					.append(TLAConstants.CR).append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT)
					.append(String.format("Format the `%s` variable as Json value.", someVar)).append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT).append(String.format(",_%sJson |->", someVar))
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT).append("LET J == INSTANCE Json")
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT)
					.append(String.format("IN J!ToJson(%s)", someVar))
					.append(TLAConstants.CR).append(TLAConstants.CR);

			localBuffer.append(TLAConstants.COMMENT)
					.append("Lastly, you may build expressions over arbitrary sets of states by")
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT)
					.append("leveraging the _TETrace operator.  For example, this is how to").append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT)
					.append("count the number of times a spec variable changed up to the current")
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT).append("state in the trace.").append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT).append(String.format(",_%sModCount |->", someVar))
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT).append("LET F[s \\in DOMAIN _TETrace] ==")
					.append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT + TLAConstants.INDENT)
					.append("IF s = 1 THEN 0").append(TLAConstants.CR);
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT + TLAConstants.INDENT)
					.append(String.format("ELSE IF _TETrace[s].%s # _TETrace[s-1].%s", someVar, someVar))
					.append(TLAConstants.CR);
			...
			localBuffer.append(TLAConstants.COMMENT + TLAConstants.INDENT).append("IN F[_TEPosition - 1]")
					.append(TLAConstants.CR);
		}

		this.tlaBuffer.append(indentString(localBuffer.toString(), 2));
		this.tlaBuffer.append(TLAConstants.CR + TLAConstants.INDENT + TLAConstants.R_SQUARE_BRACKET + TLAConstants.CR
				+ TLAConstants.CR);
	}
	 ...
	 * for each element of traceExpressionData and, if the flag addDefinitions is
	 * true,
	 ...
	 * @param attributeName
	 * @param addDefinitions      whether or not to define each identifier as the
	 *                            expression
	 */
	public void addVariablesAndDefinitions(final TraceExpressionInformationHolder[] traceExpressionData,
			final String attributeName,
	...
	/**
	 * Adds the invariant ~(P) where P is the formula describing finalState. The
	 * format
	...
	private void addInvariant(final MCState finalState) {
		...
		tlaBuffer.append(TLAConstants.INDENT).append(TLAConstants.TLA_NOT).append(TLAConstants.L_PAREN)
				.append(TLAConstants.CR);

		// The main reason why we generate an invariant compared to the much simpler
		// deadlock checking is noted in addProperties below (thank you simulation
		// mode).
		...
		// of the te spec, there might no longer be a deadlock (Worker.java doesn't
		// report
		...
		//
		// An undesired side-effect of checking _inv are occasional silly expressions
		// that
		...
		// "NULL" # [ b |-> 42, a |-> 23 ]. Thus, we conjoin TLCGet("level") #
		// Len(_TETrace)
	...
	public void addProperties(final List<MCState> trace, String specName) {
		...
		if (isStuttering) {
			...
			// simulation mode.
			// Assume any spec and an invariant such as TLCGet("level") < n for some n \in
			// Nat
			// (larger n increase the probability of a behavior with a sequence of
			// stuttering
			// steps). If the simulator happens to generate a behavior with a sequence of
			// stuttering step, the generated TE.tla will define a behavior that allows
			// infinite
			// stuttering (for each stuttering step, there will be a disjunct in the
			// disjuncts
			// of the next-state relation), which is not a deadlock. We could require the
			// simulator to run with the "-difftrace" command-line parameter, which will
			// remove
			// successive stuttering steps. However, it seems like an unnecessary
			// requirement
		...
		cfgBuffer.append(TLAConstants.INDENT + TLAConstants.COMMENT).append(ModelConfig.CheckDeadlock)
				.append(" off because of PROPERTY or INVARIANT above.")
				.append(TLAConstants.CR);
		cfgBuffer.append(TLAConstants.INDENT).append(TLAConstants.FALSE);
	}

	/**
	 * Adds the temporal property ~<>[](P) where P is the formula describing
	 * finalState.
	...
	/**
	 * Adds the temporal property ~([]<>P /\ []<>Q), where P is the formula
	 * describing finalState and
	 * Q the formula describing backToState. The formatting in the tla file is as
	 * follows:
	 * 
	 * _prop ==
	 * ~(([]<>(
	...
	/**
	 * Writes comments that will be used for associating variable names with
	 * expressions
	 * and will give the level of each expression. In particular, for each
	 * expression "expr"
	...
	public void addInitNextTraceFunction(final List<MCState> trace, final String teSpecModuleName,
			final List<String> vars, ModelConfig modelConfig) {
		/*******************************************************
		 * Add the init definition. *
		 *******************************************************/
		if (cfgBuffer != null) {
			cfgBuffer.append(TLAConstants.CR);
			cfgBuffer.append(TLAConstants.CR).append(TLAConstants.KeyWords.INIT).append(TLAConstants.CR);
			cfgBuffer.append(TLAConstants.INDENT).append(TLAConstants.TraceExplore.SPEC_TE_INIT)
					.append(TLAConstants.CR);
		}
		...
		if (trace.size() == 1) {
			...
			if (finalState.isBackToState()) {
				...
				tlaBuffer.append(TLAConstants.INDENT).append(TLAConstants.INDENT).append("   ")
						...
						.append(" i = ").append(TLAConstants.TraceExplore.SPEC_TETRACE_LASSO_END)
						.append(TLAConstants.CR);
				tlaBuffer.append(TLAConstants.INDENT).append(TLAConstants.INDENT).append("  ")
						// `_TTraceLassoStart` is a constant which contains the first state of the
						// lasso.
						.append(TLAConstants.INDENTED_CONJUNCTIVE).append("j = ")
						.append(TLAConstants.TraceExplore.SPEC_TETRACE_LASSO_START)
						.append(TLAConstants.CR);
			}
			for (String var : vars) {
		...
		tlaBuffer.append(TLAConstants.COMMENT)
				.append("to the given file in Json format. Note that you can pass any tuple").append(TLAConstants.CR);
		tlaBuffer.append(TLAConstants.COMMENT).append("to `JsonSerialize`. For example, a sub-sequence of _TETrace.")
				.append(TLAConstants.CR);
		...
		tlaBuffer.append(jsonComment + TLAConstants.INDENT + TLAConstants.INDENT)
				.append(String.format("IN J!JsonSerialize(\"%s.json\", _TETrace)", teSpecModuleName))
				.append(TLAConstants.CR);
	...
	private static String getStateConjunction(final MCState state, final String prefixConjunct) {
		if (state.isBackToState()) {
			...
			if (prefixConjunct != null) {
				formula.append(prefixConjunct).append(TLAConstants.CR).append(TLAConstants.TLA_AND)
						.append(TLAConstants.CR);
			}
