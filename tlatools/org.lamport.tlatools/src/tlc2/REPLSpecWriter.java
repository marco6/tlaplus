...
class REPLSpecWriter extends AbstractSpecWriter {
	...
	static class REPLLogConsumerStream extends ByteArrayOutputStream {
		...
		// Being wrapped in a Buffer invokes this method, we sub-optimally pass it off
		// to our real digestion below
		@Override
				...
				if (b == '\n') {
					if (inResponse) {
						if (EXPRESSION_CLOSE.equals(currentLine.toString())) {
							...
							collectedContent = allCollectedContent.substring(0,
									(length - (EXPRESSION_CLOSE.length() + 1)));
						} else {
							super.write(b);
						}
					...
					// casting to char is probably not the best thing - suffices for the majority of
					// the presumed cases
