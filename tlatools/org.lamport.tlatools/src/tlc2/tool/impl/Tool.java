 ...
 * This is one of two places in TLC, where not all messages are retrieved from
 * the message printer,
...
public abstract class Tool
   ...
   * For simulation that is not meant as a substitute of exhaustive checking for
   * too
  ...
  public Tool(String specDir, String specFile, String configFile, FilenameToStream resolver, Mode mode,
    ...
    if (mode == Mode.Simulation || mode == Mode.Executor || mode == Mode.MC_DEBUG
        || Boolean.getBoolean(TLCSTATEMUTEXT_KEY)) {
  ...
  private final void getActionsAppl(OpApplNode next, Context con, final OpDefNode actionName, CostModel cm) {
    ...
    switch (opcode) {
      case OPCODE_be: // BoundedExists
      {
        ...
        try {
          ContextEnumerator Enum = this.contexts(next, con, TLCState.Empty, TLCState.Empty, EvalControl.Clear, cm);
          if (Enum.isDone()) {
      ...
      while (!acts.isEmpty()) {
        ...
        if (!(bval instanceof BoolValue)) {
          ...
          Assert.fail(EC.TLC_EXPECTED_EXPRESSION_IN_COMPUTING,
              new String[] { "initial states", "boolean", bval.toString(), acts.pred.toString() }, acts.carPred(),
              acts.carContext());
        }
  ...
  public final StateVec getNextStates(final Action action, final Context ctx, final TLCState state) {
    ...
    this.getNextStates(action, action.pred, acts, ctx, state, s1.setPredecessor(state).setAction(action), nss,
        action.cm);
    if (coverage) {
  ...
  @ExpectInlined
  private final TLCState getNextStatesImplSubstInKind(final Action action, SubstInNode pred1, ActionItemList acts,
      Context c, TLCState s0, TLCState s1, INextStateFunctor nss, final CostModel cm) {
  ...
  @ExpectInlined
  private final TLCState getNextStatesImplApSubstInKind(final Action action, APSubstInNode pred1, ActionItemList acts,
      Context c, TLCState s0, TLCState s1, INextStateFunctor nss, final CostModel cm) {
    Subst[] subs = pred1.getSubsts();
    int slen = subs.length;
    Context c1 = c;
  ...
  private final TLCState getNextStatesAllAssigned(final Action action, ActionItemList acts, final TLCState s0,
      final TLCState s1,
    ...
    while (!acts.isEmpty()) {
      if (kind > 0 || kind == -1) {
        ...
        // Identical to default handling below (line 876). Ignored during this
        // optimization.
  ...
  protected TLCState processUnsatisfied(final TLCState ps, final SemanticNode pred, final Context c,
      IStateFunctor states, CostModel cm) {
  ...
  protected final TLCState getNextStatesApplImpl(final Action action, final OpApplNode pred, final ActionItemList acts,
      final Context c,
    ...
    if (opcode == 0) {
      ...
      if (val instanceof OpDefNode) {
        ...
        opcode = BuiltInOPs.getOpCode(opDef.getName());
        if (opcode == 0) {
          return this.getNextStates(action, opDef.getBody(), acts, this.getOpContext(opDef, args, c, true, cm, toolId),
              s0, s1, nss, cm);
        }
      }
      ...
      if (val instanceof ThmOrAssumpDefNode) {
        ...
        return this.getNextStates(action, opDef.getBody(), acts, this.getOpContext(opDef, args, c, true), s0, s1, nss,
            cm);
      }
      ...
      // TODO If all eval/apply in getNextStatesApplEvalAppl would be side-effect free
      // (ie. not mutate args, c, s0,...),
      // this call could be moved into the if(opcode==0) branch below. However,
      // opcode!=0 will only be the case if
      // OpDefNode above has been substed with a built-in operator. In other words, a
      // user defines an operator Op1,
      // and re-defines Op1 with a TLA+ built-in one in a TLC model (not assumed to be
      // common). => No point in trying
      // to move this call into if(opcode==0) because this will be the case most of
      // the time anyway.
  ...
  private final TLCState getNextStatesApplUsrDefOp(final Action action, final OpApplNode pred,
      final ActionItemList acts, final TLCState s0,
  ...
  private final TLCState getNextStatesApplSwitch(final Action action, final OpApplNode pred, final ActionItemList acts,
      final Context c, final TLCState s0,
      final TLCState s1, final INextStateFunctor nss, final CostModel cm, final ExprOrOpArgNode[] args, final int alen,
      final int opcode) {
    ...
    switch (opcode) {
      ...
      case OPCODE_lor: {
        if (PROBABILISTIC) {
          // probabilistic (return after a state has been generated, ordered is
          // randomized)
      ...
      case OPCODE_cdot: {
        if (Boolean.getBoolean(CDOT_KEY)) {
          // This assertion test is insufficient to prevent the inadvertent activation of
          // the experimental cdot feature within substitution,
          // such as refinement mapping, as the context could be an ordinary context like
          // quantification.
          // Assert.check(c.isEmpty(),
          // "The current version of TLC only supports basic action composition involving
          // no substitution/instantiation.",
          // pred, c);
  ...
  @ExpectInlined
  private final TLCState processUnchangedImpl0Arity(final Action action, final SemanticNode expr,
      final ActionItemList acts,
  ...
  /**
   * Check that <code>expr</code> is unchanged without attempting to synthesize
  ...
  @ExpectInlined
  private final TLCState processUnchangedImplTuple(final Action action, ActionItemList acts, Context c, TLCState s0,
      TLCState s1, INextStateFunctor nss,
  ...
  @ExpectInlined
  private final TLCState processUnchangedImplVar(final Action action, SemanticNode expr, ActionItemList acts,
  ...
  public TLCState evalAlias(TLCState current, TLCState successor) {
    ...
    try {
      ...
      // Compare evalAlias(TLCStateInfo current, TLCState successor, Context ctxt)
      // below.
  ...
  private TLCStateInfo evalAlias(TLCStateInfo current, TLCState successor, Context ctxt) {
    ...
    try {
      ...
      // back to the user. Throwing the exception directly was undesirable, as it
      // could
      // prevent TLC from printing an error trace after long model-checking runs.
      // Printing
      // the exception to stdout or stderr was also not an option, since it would
      // interfere
      // with the Toolbox’s parsing of output and clutter the console—especially when
      // alias evaluation fails repeatedly.
      //
      // By attaching the exception to TLCStateInfo, we defer its reporting until
      // after
      ...
      // additional state or group failed evaluations. While the resulting trace may
      // look
  ...
  @ExpectInlined
  private final Value evalImplOpArgKind(OpArgNode expr1, Context c, TLCState s0, TLCState s1, final int control,
      final CostModel cm) {
  ...
  protected final Value evalApplImpl(final OpApplNode expr, Context c, TLCState s0,
    ...
    if (opcode == 0) {
      ...
      if (val instanceof LazyValue) {
        final LazyValue lv = (LazyValue) val;
        if (s1 == null) {
          // NOTE 2023/4/7: Strictly speaking, this branch is not necessary. TLC could
          // return
          // `lv.getValue(...)` here as it does in the else-branch below. Today I
          // considered
          // removing it since it prevents LazyValue's caching benefits in some cases.
          // However, I retained it for two reasons:
          // (1) There are some debugger-related tests that make assertions about when
          // certain
          // LazyValues are evaluated and cached. Those tests fail if TLC skips this
          // branch.
          // I suspect those tests need some additional thought; if it is really important
          // that TLC not cache LazyValues during certain debugger actions, it might need
          // a
          // stronger prevention mechanism.
          // (2) I am worried that this branch serves some subtle purpose I do not yet
          // understand.
          // I do not want to remove it unless I am sure that I understand why it was here
          // to
          // begin with.
          val = this.eval(lv.expr, lv.con, s0, TLCState.Null, control, lv.getCostModel());
        } else {
          // NOTE 2023/6/15: TLC has historically had a few critical bugs due to
          // mishandling of cached
          // LazyValues. With a little refactoring we think we have finally squashed them
          // by moving
          // the safety responsibility out of this method and into the LazyValue class
          // itself. So,
          // this branch can be simply:
          return lv.getValue(this, s0, s1, control);
          // However, I am retaining the big comment that used to live here about
          // LazyValues. It contains
          // some out-of-date notes about the code, but also some useful insights that are
          // still relevant.
          //
          // ------------------------------------------------------------------------------------------------
       ...
       *********************************************************************/
        ...
        // EV#Enabled /\ EV#Prime /\ OpDeclNode is the case when A is an action (a
        // boolean
        // valued transition function (see page 312 in Specifying Systems) appearing in
        // an
    ...
    switch (opcode) {
      case OPCODE_bc: // BoundedChoose
      {
        ...
        // To fix Bugzilla Bug 279 : TLC bug caused by TLC's not preserving the
        // semantics of CHOOSE
        ...
        // inVal.normalize();
        //
        // was replaced by the following by LL on 7 Mar 2012. This fix has not yet
        // received
      ...
      case OPCODE_rfs: // RecursiveFcnSpec
      {
        ...
        FcnLambdaValue fval = (FcnLambdaValue) setSource(expr,
            new FcnLambdaValue(params, fbody, this, c, s0, s1, control, cm));
      ...
      case OPCODE_prime: {
        // MAK 03/2019: Cannot reproduce this but without this check the nested
        // evaluation
        // fails with a NullPointerException which subsequently is swallowed. This makes
        // it
        // impossible for a user to diagnose what is going on. Since I cannot reproduce
        // the
        // actual expression, I leave this commented for posterity. I recall an
        // expression
        // along the lines of:
        // ...
        // TLCSet(23, CHOOSE p \in pc: pc[p] # pc[p]')
        // ...
        // The fail statement below is obviously too generic to be useful and needs to
        // be
      ...
      case OPCODE_arrow: {
        Assert.fail(EC.TLC_ENCOUNTERED_FORMULA_IN_PREDICATE, new String[] { "a -+-> formula", expr.toString() }, expr,
            c);
        return null; // make compiler happy
      }
      case OPCODE_box: {
  ...
  @Override
  public final TLCState enabled(SemanticNode pred, Context c, TLCState s0, TLCState s1, ExprNode subscript,
      final int ail) {
  ...
  protected abstract TLCState enabledAppl(OpApplNode pred, ActionItemList acts, Context c, TLCState s0, TLCState s1,
      CostModel cm);

  protected final TLCState enabledApplImpl(OpApplNode pred, ActionItemList acts, Context c, TLCState s0, TLCState s1,
      CostModel cm) {
    if (coverage) {
      cm = cm.get(pred);
    }
    ...
    switch (opcode) {
      ...
      case OPCODE_fa: // FcnApply
      {
        Value fval = this.eval(args[0], c, s0, s1, EvalControl.setKeepLazy(EvalControl.Enabled), cm); // KeepLazy does
                                                                                                      // not interfere
                                                                                                      // with
                                                                                                      // EvalControl.Enabled
                                                                                                      // in
        ...
        if (fval instanceof FcnLambdaValue) {
          FcnLambdaValue fcn = (FcnLambdaValue) fval;
          if (fcn.fcnRcd == null) {
            Context c1 = this.getFcnContext(fcn, args, c, s0, s1, EvalControl.Enabled, cm); // EvalControl.Enabled
                                                                                            // passed on to nested
                                                                                            // this.evalAppl
        ...
        if (fval instanceof FunctionValue) {
          ...
          Value bval = fcn.apply(argVal, EvalControl.Enabled); // EvalControl.Enabled not taken into account by any
                                                               // subclass of Applicable
          if (!(bval instanceof BoolValue)) {
  ...
  protected final TLCState enabledUnchangedImpl(SemanticNode expr, ActionItemList acts,
    ...
    // e is not primed - but that also causes this code path to be taken is 23
    // below:
    //
    // -------- MODULE 23 ---------
    // VARIABLE t
  ...
  public final IMVPerm[] getSymmetryPerms() {
    ...
    // In the case where the config defines more than one set which is symmetric,
    ...
    if (argNodes.length >= values.size()) {
      // If equal, we have as many values as we have permuted sets => we have all
      // 1-element sets;
      // if greater than, then we have a heterogenous cardinality of sets, including 0
      // element sets.
  ...
  /**
   * Teases the original spec name for the set out of node and appends it to the
  ...
  /**
   * @param node
   * @return if the node represents a permutation, this will return the
   *         {@link SetEnumValue} instance contains its
   *         model values
   */
        ...
        if (TLAConstants.BuiltInOperators.PERMUTATIONS.equals(operator.getName().toString())) {
          ...
          if ((operands.length == 1)
            ...
            if (o instanceof SetEnumValue) {
              ...
              // If TLC was started with a -workers N specification, N > 1, o will be a
              // WorkerValue instance
  ...
  public final ContextEnumerator contexts(Ordering ordering, OpApplNode appl, Context c, TLCState s0, TLCState s1,
      final int control,
      CostModel cm) {
  ...
  // These three are expected by implementing the {@link ITool} interface; they
  // used
