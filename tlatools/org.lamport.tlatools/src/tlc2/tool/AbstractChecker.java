...
public abstract class AbstractChecker {
    ...
    protected static final boolean LIVENESS_STATS = Boolean
            .getBoolean(Liveness.class.getPackage().getName() + ".statistics");
     ...
     * @param preprocess
     * @param resolver
     * @param spec       - pre-built specification object (e.G. from calling SANY
    ...
    public AbstractChecker(ITool tool, String metadir, final IStateWriter stateWriter,
        ...
        if (this.checkLiveness) {
            ...
            if (LIVENESS_STATS) {
                stats = new ConcurrentBucketStatistics("Histogram vertex out-degree",
    ...
    public static final double calculateOptimisticProbability(final long numOfDistinctStates,
    ...
    /**
     * This method added by LL on 17 April 2012 to replace the use of the
     * PrintfFormat
     * method in reportSuccess.
     * 
     * Returns a string representing the decimal representation of a probability to
     * a given number of significant digits. If the input is not a probability, or
     * if
     * some error is found, then it returns the result of applying
     ...
     * x | y = an x or a y
     * 
     * @param val               - the probability represented as a long; must
     *                          satisfy 0 <= val <= 1.
     * @param significantDigits - the number of significant digits to include; must
     *                          be > 0.
     * @return
     */
    ...
    // int i = 0;
    // while (i < test.length) {
    // System.out.println("" + i + ": " + Double.toString(test[i]) + " -> " +
    // ProbabilityToString(test[i],2));
    // i++;
    // }
    // }
    ...
    public TLCStateInfo[] getTraceInfo(final TLCState from, final TLCState s) throws IOException {
        throw new UnsupportedOperationException(
                "getTraceInfo(TLCState, TLCState) not implemented for this AbstractChecker");
    }
