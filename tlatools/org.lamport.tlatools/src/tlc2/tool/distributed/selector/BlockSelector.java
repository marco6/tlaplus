...
public class BlockSelector implements IBlockSelector {
	/**
	 * A (lossy!) average block size statistic. Better not use for anything else
	 * than statistics!
	 */
	protected volatile long averageBlockCnt = 0L;
	/**
	 * A reference to {@link TLCServer} for which this {@link IBlockSelector}
	 * selects blocks
	 */
	protected final TLCServer tlcServer;

	/**
	 * Functor pattern used to make the block size configurable that gets assigned
	 * to workers
	 * 
	 * @param aTLCServer
	 */
	 ...
	 *         {@link TLCWorker} or null if no work is available
	 */
	public TLCState[] getBlocks(final IStateQueue stateQueue, final TLCWorkerRMI worker) {
		...
		// At least request a single state to guarantee StateQueue#isAvail() will be
		// triggered
