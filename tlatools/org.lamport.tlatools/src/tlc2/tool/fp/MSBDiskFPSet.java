...
public class MSBDiskFPSet extends HeapBasedDiskFPSet {
	 ...
	 * 
	 * @param maxInMemoryCapacity The number of fingerprints (not memory) this
	 *                            DiskFPSet should maximally store in-memory.
	 * @param preBits             Take the amount of DiskFPSet instance into account
	 *                            to move the index bits further to the right
	...
	protected long index(long fp, long aMask) {
		...
		// 3) Right shift by moveBy (the bits which have previously been zeroed because
		// of mask.
	...
	public class MSBFlusher extends Flusher {
		...
		protected void prepareTable() {
			...
			// had been flush in a previous flush operation and zero/0 (which is invalid
			// anyway).
		}
		 ...
		 * (non-Javadoc)
		 * 
		 * @see tlc2.tool.fp.DiskFPSet#mergeNewEntries(long[], int,
	...
	public static class TLCIterator {
		 ...
		 * 
		 *                                   Pre-condition: Each bucket is sorted in
		 *                                   ascending
		 *                                   order!
		 */
		public long getLast() {
