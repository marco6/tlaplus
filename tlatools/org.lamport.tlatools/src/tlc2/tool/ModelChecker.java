...
// changes
// in the type hierarchy. multiple methods has been pulled up to the super
// class.
// unused constructors has been removed
// the class now contains only the parts, which are different from the
// DFIDModelChecker
// the name resolver and support for the external specification object has been
// added
public class ModelChecker extends AbstractChecker {
	...
	public ModelChecker(ITool tool, String metadir, final IStateWriter stateWriter, boolean deadlock, String fromChkpt,
			final Future<FPSet> future, long startTime)
			throws EvalException, IOException, InterruptedException, ExecutionException {
	...
	public ModelChecker(ITool tool, String metadir, final IStateWriter stateWriter, boolean deadlock, String fromChkpt,
		...
		this(tool, metadir, stateWriter, deadlock, fromChkpt, startTime);
		this.theFPSet = FPSetFactory.getFPSet(fpSetConfig).init(TLCGlobals.getNumWorkers(), metadir,
				tool.getRootName());
		...
		try {
			...
			if (this.errState == null) {
				...
				try {
					...
					// Do not replace the actual error code, such as assert violation, with
					// TLC_NESTED_EXPRESSION.
	 ...
	 */
	private final boolean doNext(final ITool tool, TLCState curState, final SetOfStates liveNextStates,
	...
	private final boolean doNextCheckInvariants(final ITool tool, final TLCState curState, final TLCState succState)
	...
	private final boolean doNextCheckImplied(final ITool tool, final TLCState curState, final TLCState succState)
			throws IOException, WorkerException, Exception {
		int k = 0;
		try {
	...
	final boolean doNextSetErr(TLCState curState, TLCState succState, boolean keep, int ec, String param)
	...
	final boolean doNextSetErr(TLCState curState, TLCState succState, Action action)
			throws IOException, WorkerException {
		synchronized (this) {
	...
	public final int doPeriodicWork() throws Exception {
		...
		if ((!this.checkLiveness || runtimeRatio > TLCGlobals.livenessRatio || !liveCheck.doLiveCheck())
	...
	public final void printSummary(boolean success, final long startTime) throws IOException {
		...
		// The depth used to only be reported on success, but this seems bogus since TLC
		// reports
	 ...
	 * commenting. I presume
	 * he really mean "ProGRess calculation", since this seems to be where the
	 * coverage
	 * and progress information is written. The method writes the progress
	 * information,
	 * prints the coverage only if count = 0, and then waits until it's time to
	 * print
			...
			try {
				...
				if (!tool.isGoodState(curState)) {
					MP.printError(EC.TLC_INITIAL_STATE,
							new String[] { "current state is not a legal state", curState.toString() });
