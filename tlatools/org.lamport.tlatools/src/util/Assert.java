...
/**
 * A toolkit for checking conditions and throwing unchecked exceptions if they
 * are not met.
 * 
...
public class Assert {
    ...
    public static void fail(int errorCode, String[] parameters, SemanticNode expr) {
        if (expr == null) {
            ...
            throw new TLCDetailedRuntimeException(errorCode, parameters, MP.getMessage(errorCode, parameters), expr,
                    Context.Empty);
        }
    }
    ...
    public static void fail(int errorCode, String parameter, SemanticNode expr, Context ctxt) {
        if (expr == null) {
            ...
            throw new TLCDetailedRuntimeException(errorCode, new String[] { parameter },
                    MP.getMessage(errorCode, parameter), expr, ctxt);
        }
    }
    ...
    public static void check(boolean condition, String reason, SemanticNode expr, Context ctxt)
    ...
    /**
     * Checks whether the condition is true. Throws an unchecked exception if
     * otherwise
     * 
    ...
    /**
     * The following method added by LL on 5 Oct 2009 because, for some unknown
     * reason,
     * javacc seems to have begun generating code that requires such a method.
     * 
     * Someone removed it (probably Simon Z), presumably because it was no longer
     * needed
     * by javacc. However, it was added again by LL on 7 April 2012 to replace
     * check(boolean, int) above when it was called with error code EC.GENERAL,
     * which
    ...
    public static class TLCDetailedRuntimeException extends TLCRuntimeException {
        ...
        public TLCDetailedRuntimeException(int errorCode, String[] parameters, String message, SemanticNode expr,
                Context ctxt) {
