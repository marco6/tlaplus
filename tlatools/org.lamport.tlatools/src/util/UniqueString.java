 ...
 * only once for the entire run. This leads to a major speed-up in string
 * comparison.<br>
 * 
 ...
 * data holder inside of the {@link InternTable}. The latter is organized that
 * way, that it holds an
 * array of UniqueStrings. Each UniqueString that is created is put at most once
 * into the table, and holds
 * the information of its location in the table. The member variable
 * {@link UniqueString#s} is used to represent
 * the content, the member variable {@link UniqueString#tok} holds the position
 * inside of the {@link InternTable}.
 ...
 * </ul>
 * <br>
 * In addition, there exist two types of externally stored tables: the array of
 * values of state variables
 * in subclasses of {@link TLCState}, and array of operator definitions in
 * {@link Defns}. The main scheme
 * behind the storage of the objects in these arrays is that the value of a
 * variable / operation
 * definition with name, identified by a UniqueString <code>foo</code> is stored
 * at position, in the array, that is
 * stored inside of <code>foo</code>, using the instance member
 * {@link UniqueString#loc}. Note, that multiple instances
 * of TLCState store values of variables on the same position in their arrays
 * and only one Defn instance
 * exist. This is OK, because the state variable are global.
 * <br>
 * In order to distinguish between the index in the state variable array and
 * index of operator definition array,
 * the number of state variables defined in the specification is maintained in
 * the static member {@link UniqueString#varCount}.
 ...
 * </ul>
 * <br>
 * Finally, there are two methods responsible for marshaling/un-marshaling and
 * convenience methods to put and get unique
...
public final class UniqueString implements Serializable {
    ...
    // since it is only needed in Spec#processSpec and the setter is called from
    // there
     ...
     * Delivers the difference in positions inside of the table, the unique strings
     * are stored in
     * 
     * @param uniqueString
     * @return
    ...
    /**
     * Since uniqueness is guaranteed, the equals is a high performance reference
     * comparison
     */
