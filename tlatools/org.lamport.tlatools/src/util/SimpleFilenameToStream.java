...
public class SimpleFilenameToStream implements FilenameToStream {
   ...
   * installation path and instead the path supplied using the system property
   * TLA_LIBRARY
  ...
  /**
   * August 2014 - TL
   * added an informative method for returning the actual path used by this
   * resolver
   */
  public String getFullPath() {
    return userFileResourceLocator.describeSearchLocations() + ", "
        + standardLibraryResourceLocator.describeSearchLocations();
  }

  /**
   * Create a locator that searches {@link ToolIO#getUserDir()} if it is non-null,
   * otherwise searches the JVM's current
  ...
  /**
   * Parse a string as an array of paths separated by
   * {@link FileUtil#pathSeparator}. Returns an empty array if
   * <code>pathList == null</code>.
   *
   * @param pathList a string containing paths separated by
   *                 {@link FileUtil#pathSeparator}
   ...
   *
   * <p>
   * The name is treated as a filename, not a TLA+ module name; if you are looking
   * for the Integers module, pass <code>"Integers.tla"</code>.
   *
   * <p>
   * The first fully-qualified name that refers to a file that actually
   * exists is the File returned; but if none exists, a non-existent file is
   * returned
  ...
  private final TLAFile locate(String name) {
    ...
    // Small bit of awkwardness here: to preserve the historical FilenameToStream
    // API, this method must return a
    // nonexistent file if the requested one could not be located. The file we
    // return will be used in error messages
    // about the missing file, so we should return one that looks plausible, but we
    // can't return one that MIGHT exist,
    // because the result will be read if it does! The approach below correctly
    // synthesizes a non-existent
    // believable-looking path, but relies on the fact that ToolIO.getUserDir() is
    // in the search paths used by
    // userFileResourceLocator---this file does not exist because it has certainly
    // already been searched.
    //
    // In the future, we might improve this pattern by deprecating FilenameToStream
    // and replacing it with a better
  ...
  /**
   * Use the given locator to locate the given file, copying it to the filesystem
   * if necessary.
   *
   * @param locator         the locator
   * @param name            the file name
   * @param isLibraryModule whether this file should be marked as a standard
   *                        library module
  ...
  private TLAFile locateAndCopyToFilesystemIfNecessary(ResourceLocator locator, String name, boolean isLibraryModule) {
    try {
      ...
      // Reminder: URL schemes (or "protocols") are supposed to be lower-case, but we
